...
day routes, logistics panels, and interactive Leaflet maps.
...
Tracked itinerary JSON
        |
        v
TypeScript data helpers ---> App Router server pages ---> HTML/RSC response
        |                              |
        |                              v
...
There is currently no application database, authentication service, or custom
API layer. Repository JSON is the canonical trip-data source.
...
│   ├── lib/                 Data access and domain helper functions
...
Formatting belongs at the display boundary. Domain values remain machine
readable: dates use ISO strings, currency uses a code plus numeric amounts, and
coordinates remain numbers.
...
operational trip planning. `src/types/map.ts` defines the reusable mapping
contract.
